    ...
    <div>
      <div className="bg-white pt-16  px-60">
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          ...
          {Object.keys(products).length == 0 && <div className="text-base font-sans font-medium text-center">
            Sorry items are currently out of stock, Stay tuned!!
            </div>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "handbag" });
  let handbag = {};
  for (let item of products) {
    if (item.title in handbag) {
      if (
        !handbag[item.title].color.includes(item.color) &&
        item.availableqty > 0
      ) {
        handbag[item.title].color.push(item.color);
      }
      if (
        !handbag[item.title].size.includes(item.size) &&
        item.availableqty > 0
      ) {
        handbag[item.title].size.push(item.size);
      }
    } else {
      handbag[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableqty > 0) {
        handbag[item.title].color = [item.color];
        handbag[item.title].size = [item.size];
      }else{
        handbag[item.title].color = [];
        handbag[item.title].size = [];
      }
    }
  }

  return { props: { products: JSON.parse(JSON.stringify(handbag)) } };
}
