...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in book) {
      if (
        !book[item.title].color.includes(item.color) &&
        item.availableqty > 0
      ) {
        book[item.title].color.push(item.color);
      }
      if (
        !book[item.title].size.includes(item.size) &&
        item.availableqty > 0
      ) {
        book[item.title].size.push(item.size);
      }
    } else {
      book[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableqty > 0) {
        book[item.title].color = [item.color];
        book[item.title].size = [item.size];
      }else{
        book[item.title].color = [];
        book[item.title].size = [];
      }
    }
  }

  return { props: { products: JSON.parse(JSON.stringify(book)) } };
}
