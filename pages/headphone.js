...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in headphone) {
      if (
        !headphone[item.title].color.includes(item.color) &&
        item.availableqty > 0
      ) {
        headphone[item.title].color.push(item.color);
      }
      if (
        !headphone[item.title].size.includes(item.size) &&
        item.availableqty > 0
      ) {
        headphone[item.title].size.push(item.size);
      }
    } else {
      headphone[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableqty > 0) {
        headphone[item.title].color = [item.color];
        headphone[item.title].size = [item.size];
      }else{
        headphone[item.title].color = [];
        headphone[item.title].size = [];
      }
    }
  }

  return { props: { products: JSON.parse(JSON.stringify(headphone)) } };
}
