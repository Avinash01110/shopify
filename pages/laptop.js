...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in laptop) {
      if (
        !laptop[item.title].color.includes(item.color) &&
        item.availableqty > 0
      ) {
        laptop[item.title].color.push(item.color);
      }
      if (
        !laptop[item.title].size.includes(item.size) &&
        item.availableqty > 0
      ) {
        laptop[item.title].size.push(item.size);
      }
    } else {
      laptop[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableqty > 0) {
        laptop[item.title].color = [item.color];
        laptop[item.title].size = [item.size];
      }else{
        laptop[item.title].color = [];
        laptop[item.title].size = [];
      }
    }
  }

  return { props: { products: JSON.parse(JSON.stringify(laptop)) } };
}
